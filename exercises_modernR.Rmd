...
#### a) Read the dataset `restaurants.csv`

Complete the code in the code cell below to read the data into
`data_restaurants.csv`. *Tip: Use `head(data_restaurants)` to display the
result.*
...
Read the menus data file `menus.xlsx` (Excel file!). Complete the code in the
...
Save the tibble `data_menus` to a `.csv` file. Use `;` as delimiter. 
...
Date and time variables can be very challenging to work with in programming languages (including R). 
