...
#### What we will do:
- *During presentations*: Answer your question right there, right then.
...
- We assign a value to `m`:

  ```{r}
  m <- 14
  ```

- In our if-statement, we:
  - test whether `m` is larger than 10 (the _condition_)
...
- Then we look at what `number class` is:
  ```{r}
  print(number_class)
  ```
