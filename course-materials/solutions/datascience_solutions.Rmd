...
```{r}
penguins_subset_3 <- penguins_subset_2 %>%
  mutate(culmen_ratio = Culmen_Length_mm / Culmen_Depth_mm)
```

# Exercise 7

Use %>% (the pipe operator) to combine the code you wrote in Exercises 4, 5 and 6. 

Make a workflow that starts with the data `penguins` and subsequently applies your `filter`, `select` and `mutate` operations. Make sure to remove references to the original dataframe in subsequent operations, as this is now provided through the %>% operator.

```{r}
penguins_subset_4 <- penguins %>% 
  filter(!is.na(Sex)) %>%
  select(Individual_ID, Species, Sex, Island, starts_with("Culmen")) %>%
  mutate(culment_ratio =  Culmen_Length_mm / Culmen_Depth_mm)
```
